...
file_path = '/run/media/ed/My Passport/ed/aug-1-alpha/' # folder with H5 data files
runs_alpha_1us = []
files = [
'Run_1690911731.hdf5', 'Run_1690912805.hdf5', 'Run_1690913161.hdf5', 'Run_1690913708.hdf5',
                       'Run_1690914711.hdf5', 'Run_1690915132.hdf5', 'Run_1690915713.hdf5',
...
#%%
alpha_data.plot_alpha(out_file='2023August01_Alpha.csv')

alpha_data.plot_num_det_photons()

##%% values based on Wesley's APS slides
N = 5.49/(19.6E-6)
PTE_noCU_spec = .003353
PTE_noCU_dif = .001979
alpha_data.plot_PDE(N*PTE_noCU_spec, out_file='2023August01_Alpha.csv', legtext='.003353 (Specular Silicon Reflector)')
...
data = 'amps'
df = pd.read_csv('2023August01_Alpha.csv').sort_values('ov')#.head(10).tail(9)
data_x = df['ov']
data_x_err = df['ov error']
data_y = df[data]
data_y_err = df[data+' error']

df10 = pd.read_csv('2023August10_Alpha.csv').sort_values('ov')#.head(10).tail(9)
d10_x = df10['ov']
...
ax.errorbar(data_x, data_y, xerr = data_x_err, yerr = data_y_err, markersize = 8, fmt = '.',
            color = 'tab:purple', label='Silicon Reflector')
