...
upper_limit = [0.6, 0.6, 0.72, 1.5, 1.5, 1.6, 1.5, 1.5, 1.5, 1.5]
proms = [0.0046, 0.0046, 0.0049, 0.0049, 0.0049, 0.0049, 0.0049, 0.007, 0.0072, 0.0072]
...
for file in range(len(files)):
    ...
    run = RunInfo([path + files[file] + '.hdf5'], do_filter = True, upper_limit = upper_limit[file], baseline_correct = True, prominence = proms[file], is_led = True)
    runs_spe.append(run)
...
def get_subtract_hist_mean(bias, data1, data2, numbins = 1000, plot = False):
    ...
    # subtracted_counts = subtracted_counts[6:]
    # centers = centers[6:]
    # counts1 = counts1[6:]
    # counts2 = counts2[6:]
    ...
    # plt.step(centers,norm_subtract_hist)
...
#%%
get_subtract_hist_mean(test.bias,test.all_led_peak_data,test.all_dark_peak_data, plot = True, numbins = 2000)
#%%
get_subtract_hist_mean(runs_spe[1].bias,runs_spe[1].all_led_peak_data,runs_spe[1].all_dark_peak_data, plot = True, numbins = 4000)
#%%
for run in runs_spe:
    get_subtract_hist_mean(run.bias,run.all_led_peak_data, run.all_dark_peak_data, plot = True, numbins = 4000)

#%%#%% variable peaks
range_lows2 = [0.0046,0.0048,0.0048,0.0048,0.0049,0.0052,0.0061,0.0068, 0.007, 0.0071] 
centers2 = [0.005,0.00555,0.006,0.0068,0.0075,0.008,0.0086, 0.0092, 0.0099, 0.0101] #
range_highs2 = [0.026,0.0264,0.034,0.031,0.0337,0.035,0.047, 0.0419, 0.0435, 0.0465]
numpeaks = [4,4,5,4,4,4,5,4,4,4]
...
info_spe.date = runs_spe[n].date
info_spe.temperature = T
info_spe.bias = runs_spe[n].bias
info_spe.baseline_numbins = 50
info_spe.peaks_numbins = 150 #unused
info_spe.data_type = 'h5'
wp = WaveformProcessor(info_spe, run_info_self = runs_spe[n], run_info_solicit = run_spe, baseline_correct = True,  range_low = range_lows2[n], range_high = range_highs2[n], center = centers2[n], peak_range = (1,numpeaks[n]), status = 1)
...
for i in range(1,len(runs_spe)):
    ...
    info_spe.data_type = 'h5'
    wp_spe = WaveformProcessor(info_spe, run_info_self = runs_spe[i], run_info_solicit = run_spe, baseline_correct = True, range_low = range_lows2[i], range_high = range_highs2[i], center = centers2[i], peak_range = peak_num)
...
for run in campaign_spe2:
    run.plot_peak_histograms(log_scale = False, savefig=True, path = 'D:/Xe/AnalysisScripts/No Source Run/July/July13_LXe_gauss_'+str(run.info.bias)+'.png')
    run.plot_spe(savefig=True, path = 'D:/Xe/AnalysisScripts/No Source Run/July/July13_LXe_spe_'+str(run.info.bias)+'.png')
    plt.close()
#%%
...
#%%
path = 'D:/Raw Results/Correlated Avalanche Data/July13_171K_LXe_CA.csv'
path_spe = 'D:/Raw Results/Breakdown Voltage/SPE Amplitudes/July13_171K_SPE.csv'
# filtered_spe.plot_spe(in_ov = False, absolute = False, out_file = path_spe)
filtered_spe.plot_CA(out_file = path)
...
path = 'E:/Xe/AnalysisScripts/Nitrogen Dark Run 2022/151K'
for w in range(len(campaign_spe)):
    folder = path
    bias = biases[w]
    # wp_spe.plot_both_histograms(savefig = True, path = folder +'baseline_' + str(T)[:3] + '_' + str(bias)[:2] + '.png')
    campaign_spe[w].plot_peak_histograms(savefig = True, path = folder +'gauss_' + str(T)[:3] + '_' + str(bias)[:2] + '.png')
    campaign_spe[w].plot_spe(savefig = True, path = folder + 'line_' +str(T)[:3] + '_' + str(bias)[:2] + '.png')
